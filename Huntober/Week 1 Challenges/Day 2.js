...
// Afterwards, remove all the instances of the word 'Eek!' in the remaining string please (case sensitive, of course).
...
// Do what it says and save the answer somewhere safe for tomorrow's challenge - it's key info.
...
function lolCats(str) {
    // Convert the string into an array and find the index of the first instance of 'c'
    indexOfC = str.split('').findIndex((element) => element === 'c')

    // Slice the array starting from the first element following the first 'c' all the way up till the end
    // Replace every instance of "Eek!" with an empty string
    // Afterwards, split the string to make it an array, reverse it, and then join it to make it a string again
    newArr = str.slice(indexOfC + 1).replace(/Eek!/g, '').split('').reverse().join('')
    ...
    return newArr.split('v')
}

// Result:
["S.A-#IYbSEkiQS5Sn@tq!CcG;+:^myl%CJcWzUZ;",
    "pZQP*nH1KA*GRFI+B/xHR:><%&4.zf*2+Ey0x-#Kci LGcoOz3XKA)SAZ/j>T3sT=~kVs !6=B_Yei~@5~Gn)Q?Q7zI4yqfgXeTsdi9ga43?w^oC~KMqw8i02WHHm%-0QdTBJEum%@R_?,dJk 2t5QmgZRN8xW$fQ@Ge>tub)Ep1YjX()dpD3ru1(Q<8m?",
    "aotN<SqFz-7z8fDWj7s|=UR*E:/CYLzj*?K:uCUmhkASg8;H^/%<HU%>=hhPpLPdb|fBAM",
    "cA=)#4HH=Q^4nVHx^w,!T<11M~RQ.iI7Xk?lbtc d35csP&k1Xk2zk87NF^w691a4_xu",
    "eA=)#4HH=Q^4nVHx^w,!T<11M~RQ.iI7Xk?lbtc d35csP&k1Xk2zk87NF^w691a4_xu"]
